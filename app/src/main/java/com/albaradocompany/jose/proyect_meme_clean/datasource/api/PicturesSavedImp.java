...
import com.albaradocompany.jose.proyect_meme_clean.datasource.api.retrofit.PicturesSavedService;
...
public class PicturesSavedImp implements GetPicturesById, Callback<PictureResponse> {
    ...
    public void getPictures(Listener listener) {
        ...
        Retrofit retrofit = new Retrofit.Builder().baseUrl(BuildConfig.BASE_URL_DEFAULT)
                ...
                .build();
        PicturesSavedService service = retrofit.create(PicturesSavedService.class);
        service.getPicturesSaved(userId).enqueue(this);
    }
